 ...
 *   7. Sustainability               -> /api/sustainability (dedicated tab, cached AI eco tips)
 *                                       plus general eco tips in the chat assistant
 *
 *  SECURITY NOTES
 *  ----------------
 ...
 *     STAFF_SIGNUP_CODE          Shared secret required to self-register a 'staff' account via
 *                                 /api/auth/signup. Unset by default, which disables staff
 *                                 self-signup entirely (fans can still sign up).
 *
...
// ----------------------------------------------------------------------------
// 12b. ROUTES — SUSTAINABILITY TIPS
// ----------------------------------------------------------------------------
...
app.get('/api/sustainability', async (req, res, next) => {
  try {
    const cacheKey = 'sustainability:tips';
    const cached = cache.get(cacheKey);
    if (cached) return res.json(cached);
    ...
    if (FEATURES.gemini) {
      ...
      try {
        ...
        // Keep the static fallback tips if the model didn't return valid JSON.
      }
    }
    const payload = { tips, generatedAt: new Date().toISOString() };
    cache.set(cacheKey, payload, 30 * 60_000); // tips don't need to change often
    res.json(payload);
  } catch (err) { next(err); }
});
